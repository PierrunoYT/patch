...
- OpenAI and Anthropic have basic executable routes, DeepSeek requests are
  ...
  `gpt-4o-mini` defaults, DeepSeek Reasoner weak/editor routing, and exact
...
- The eight ancillary feature families have explicit dispositions below.
  ...
  installed packed executable. This ancillary acceptance does not establish
  broader parity.
...
- [x] Implement typed commands for `/add`, `/drop`, `/read-only`, `/attach`,
  `/help`, `/settings`, `/report`, `/diff`, `/tokens`, `/ls`, `/clear`, `/model`, `/chat-mode`,
  `/run`, `/web`, `/test`, `/lint`, `/commit`, `/undo`, `/copy`, `/paste`, and
  `/exit`. `tests/advertised-commands.test.ts` keeps this 27-command documented
