...
| Commands/config | Much narrower option and command surface; `/help` now provides bounded offline search rather than aider's model-backed help, while `/settings` and `/report` remain planned. |
