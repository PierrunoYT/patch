# Aider source inventory — 2026-09-15 (`cee39ed`)

## Reproducible boundary

This inventory supports the [current parity audit](aider-parity-audit-2026-09-15-cee39ed.md).
It compares Patch `cee39ed41330eca755b9c7c65084abccefce90aa` with canonical
aider `5dc9490bb35f9729ef2c95d00a19ccd30c26339c`. Both worktrees were clean before
review. The aider checkout remained outside this repository at
`../aider-upstream`.
...
- **partial** — Patch implements a useful selected subset or different product
  contract; this does not itself identify an open defect.
- **unported** — no Patch production equivalent exists and no explicit product
  disposition has replaced it.
- **non-goal** — intentionally absent unless a future product decision changes.
...
| `aider/args.py`               | partial            | `src/program.ts` and `src/config/bootstrap.ts`; Patch deliberately exposes a smaller exact option set.                                                                          |
| `aider/args_formatter.py`     | non-goal           | Commander supplies Patch's smaller help surface; grouped aider formatting is not reproduced.                                                                                    |
| `aider/commands.py`           | partial            | `src/commands/**` and concrete dispatch provide 28 exact commands versus 43 `cmd_*` methods plus aliases upstream. Omitted aliases/breadth have explicit non-goal dispositions. |
| `aider/copypaste.py`          | partial            | `src/io/integrations.ts`; bounded cancellable text-only clipboard behavior with no image probing.                                                                               |
| `aider/deprecated.py`         | non-goal           | No legacy aider-option compatibility layer.                                                                                                                                     |
| `aider/diffs.py`              | non-goal           | Patch retains full-content previews and a separate bounded Git `/diff`; computed preview hunks require the rejected Rich renderer project.                                      |
| `aider/dump.py`               | non-goal           | No debug object-dump command in the supported surface.                                                                                                                          |
| `aider/editor.py`             | partial            | `src/io/editor.ts`; shell-free temporary draft editing and Windows-safe tokenization are production-wired, but child wait/cancellation and edited-file readback are unbounded.                                                                |
...
| `aider/gui.py`                | non-goal           | The authenticated HTTP/SSE API is not a browser GUI; a second GUI product surface is explicitly out of scope.                                                                   |
...
| `aider/help_pats.py`          | non-goal           | Semantic-help corpus is unused by Patch's bounded local help.                                                                                                                   |
| `aider/history.py`            | implemented        | `src/core/chat-summary.ts`; bounded input and weak-to-main fallback are Patch hardening.                                                                                        |
| `aider/io.py`                 | partial            | `src/input.ts` and `src/io/**`; Patch's minimal streaming/sanitized terminal is wired, while general text reads retain no ancestor identity or byte bound and Rich/Vi/broad controls are non-goals.                                            |
| `aider/linter.py`             | partial            | `src/process/configured-checks.ts`; exactly one explicit bounded lint and test command, with inference/bundled linters rejected.                                                |
...
| `aider/mdstream.py`           | partial            | `src/io/render.ts`; variable-length fences are fixed, but Rich tables/wrapping/unstable-tail rendering are non-goals.                                                           |
| `aider/models.py`             | partial            | `src/models/**`; narrow packaged catalog and controls. All six bundled profiles currently inherit a fixed 1,024-token history threshold rather than aider's input-limit-derived threshold.                                                    |
| `aider/onboarding.py`         | non-goal           | No automatic account probing, OAuth, or credential persistence.                                                                                                                 |
| `aider/openrouter.py`         | non-goal           | OpenRouter is unsupported and implicit startup metadata fetch/cache is intentionally absent.                                                                                    |
| `aider/prompts.py`            | partial            | `src/resources/prompts.ts` and strategy prompts; English-only, shorter Patch templates.                                                                                         |
| `aider/reasoning_tags.py`     | implemented        | `src/core/reasoning.ts` plus attempt-atomic reclassification keep close-only reasoning out of display, history, continuation, and parsing.                                      |
| `aider/repo.py`               | partial            | `src/repository/git.ts`; supported Git workflow plus stronger literal-path and undo rules.                                                                                      |
| `aider/repomap.py`            | partial            | `src/context/**`; eleven languages and scoped ranking/rendering evidence. Cache hashing currently precedes the extractor's 4 MiB source bound.                                                                                                |
| `aider/report.py`             | partial            | `src/commands/report.ts`; intentionally local, bounded, reviewable, and upload-free.                                                                                            |
| `aider/run_cmd.py`            | partial            | `src/process/**`; explicit approval and bounded non-PTY execution, but PTY transcript accumulation is unbounded.                                                                 |
| `aider/scrape.py`             | partial            | `src/interfaces/url-fetcher.ts`; explicit URL only, DNS-pinned, bounded, no production browser rendering. Nested discarded HTML tags are currently mishandled.                                                                                |
...
| `aider/voice.py`              | partial / non-goal | Optional `@pierrunoyt/patch/voice` embedding helper is wired, but active ffmpeg cancellation has no forced-termination deadline; CLI/device UX is a non-goal.                                                                                   |
| `aider/waiting.py`            | non-goal           | No aider-style waiting spinner in the dependency-free terminal contract.                                                                                                        |
| `aider/watch.py`              | partial            | `src/interfaces/watch-mode.ts`; different marker/authorization policy with ancestor-swap containment fixed.                                                                     |
...
| `aider/coders/help_coder.py`                    | non-goal    | Replaced by bounded local `/help`; no model-backed help coder.                                                                                                   |
| `aider/coders/help_prompts.py`                  | non-goal    | Not used by Patch's local help.                                                                                                                                  |
| `aider/coders/patch_coder.py`                   | partial     | `src/edits/patch.ts`; scoped typed actions with stronger conflict rejection, but inherited missing-sentinel/truncated-output tolerance remains.                                 |
...
| `aider/coders/udiff_coder.py`                   | implemented | Selected bounded recovery, physical-line fence parsing, and validated insertion ranges are production- and package-tested, with intentional ambiguity hardening. |
| `aider/coders/udiff_prompts.py`                 | partial     | Shorter Patch protocol prompt.                                                                                                                                   |
| `aider/coders/udiff_simple.py`                  | non-goal    | Prompt-only variant adds no parser behavior beyond Patch's supported `udiff`.                                                                                    |
| `aider/coders/udiff_simple_prompts.py`          | non-goal    | Companion prompt-only identity is intentionally absent.                                                                                                          |
...
| ---------------------------------------------------------------------------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `aider/resources/model-metadata.json`                                                    | partial     | `src/resources/model-metadata.json5`; selected bundled values are packaged, and DeepSeek limits match pinned aider exactly.              |
| `aider/resources/model-settings.yml`                                                     | partial     | `src/resources/model-settings.yml`; selected profiles are packaged, including matched `gpt-4o-mini` and DeepSeek Reasoner role defaults. |
...
The remaining 46 pinned queries are unported and not selected for this release:
...
The alternate-directory copies for languages Patch already supports remain
unported because Patch selected and attributed the other pinned query; language
support is not evidence that both upstream files were ported.
...
`pyproject.toml`, `pytest.ini`, ten upstream workflows, Python/Docker release
...
This inventory makes upstream-file coverage reproducible. Inventory
completeness is not semantic parity: it does not prove semantic equivalence
inside a file, detect every interaction, or make the
64-entry direct-derivation ledger complete by construction. The ledger answers
“which Patch files identify themselves as direct adaptations”; this inventory
separately answers “what happened to every pinned aider product file.” Both must
be maintained when the pinned revision changes.
