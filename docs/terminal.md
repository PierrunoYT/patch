...
`aider/coders/wholefile_coder.py:136–140`, Patch intentionally does not use the
upstream `difflib.unified_diff(..., n=5)` preview path. Computed preview hunks
are a current-release non-goal. The separate Git-backed `/diff` command displays
Git's selected-file patch and does not change edit-preview rendering.
