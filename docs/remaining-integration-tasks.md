...
| Repository maps | partial with scoped rendering | An eleven-language map refreshes tracked inventory per turn and has exact upstream tags for each committed language sample. Ordinary turns fall back from selected-file maps to hinted and then unhinted global maps; private context selection force-refreshes an expanded map with original-request identifier hints. Shipped grammars use generic pinned parent-scope/header/elision behavior, and fitting uses a selected tokenizer where reliable with a documented conservative fallback. Broader ranking/personalization/lexical fixtures and executable provider-turn evidence remain incomplete. |
...
- [ ] **P1 — Complete repository-map evidence.** Add independent numeric
  ranking/personalization and lexical-fallback fixtures, then exercise broader
