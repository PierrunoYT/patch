...
- [x] **P2 — Make `FfmpegVoiceRecorder` honor a pre-aborted signal directly.**
  **Status:** fixed 2026-09-15. The exported adapter calls
  `AbortSignal.throwIfAborted()` before constructing a child. Its direct test
  uses the Node executable as a deterministic ffmpeg stand-in and proves a
  pre-aborted call preserves the reason, creates no process side effect, and
  installs no abort listener. This strengthens pinned aider's synchronous voice
  path without widening Patch's deferred CLI/device scope.
...
- [ ] **P2 -- Add read-only inspection commands selected for Patch's product
  ...
  `/settings`, selected-file `/diff`, and `/tokens`. The token report rebuilds
  ...
  message; map controls and context copying remain product decisions rather
...
**Status:** Every one of Patch's 27 named commands reaches a concrete effect.
...
- [x] Add an application-owned dispatcher for `/add`, `/attach`, `/drop`,
  `/read-only`, `/help`, `/settings`, `/report`, `/diff`, `/tokens`, `/ls`, `/clear`,
...
equal to the parser/completion inventory and drives all 27 effects through one
...
Clipboard utilities are bounded and cancellable, and editor commands retain
Windows separators. Variable-fence language rendering remains an open
correctness defect. True Vi modal editing and broader Rich renderer fidelity
remain deferred; helper capabilities must not be read as additional executable
behavior.
...
source-identifier input paths, but the terminal exit is blocked by variable-fence
language rendering. Recall, multiline, Windows-safe editor splitting, bounded
clipboard execution, explicit PTY dispatch, shell completions, and notification
timing remain executable rather than helper-only. Broader Rich renderer fidelity
...
- [x] Correct Phase 8/9 checkboxes after terminal sanitization, rich input, web
  ...
  their exits for renderer correctness and the ffmpeg pre-abort contract;
  Windows tokenization, clipboard bounds, and read containment are now fixed,
  while richer rendering, GUI, and CLI voice remain deferred.
...
- [x] every named Patch slash command through its exercised application effect
  ...
  equality and executes all 27 effects in a real temporary Git repository,
